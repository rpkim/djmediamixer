...
void World::bodyCollide(Body *bA, Body *bB, std::vector<BodyCollisionInfo> &infoList)
{
    ...
    for (int i = 0; i < bApmCount; i++)
    {
        ...
        {
            infoSame.penetration = Vector2::fsqrtf(infoSame.penetration);
            infoList.push_back(infoSame);
        }
        else
        {
            infoAway.penetration = Vector2::fsqrtf(infoAway.penetration);
...
void World::_handleCollisions()
{
	...
	//mCollisionList counts
	int mCollisionListC = mCollisionList.size();

    for (int i = 0; i < mCollisionListC; i++)
    {
