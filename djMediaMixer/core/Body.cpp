...
void Body::derivePositionAndAngle(float elaspsed)
{
    ...
    center.X /= mPointMassesC;
    center.Y /= mPointMassesC;
    ...
    for (int i = 0; i < mPointMassesC; i++)
    {
        ...
        else
        {
			...
			if ((fabs(diff) > thePI) && (thisSign != originalSign))
            {
                thisAngle = (thisSign == -1) ? (thePI + (thePI + thisAngle)) : ((thePI - thisAngle) - thePI);
            }
        }
    ...
    if (fabs(angleChange) >= thePI)
    {
        if (angleChange < 0.0f)
            angleChange = angleChange + (thePI * 2);
        else
            angleChange = angleChange - (thePI * 2);
    }
...
float Body::getClosestPointOnEdge(Vector2 pt,unsigned int edgeNum,  Vector2 &hitPt,  Vector2 &normal,  float &edgeD)
{
    hitPt =  Vector2();
    hitPt.X = 0.0f;
    hitPt.Y = 0.0f;
    ...
    if (edgeNum < (mPointMasses.size() - 1))
	...
	float edgeLength = Vector2::fsqrtf(((E.X * E.X) + (E.Y * E.Y)));
...
float Body::getClosestPointOnEdgeSquared(Vector2 pt,unsigned int edgeNum, Vector2 &hitPt, Vector2 &normal, float &edgeD)
{
    hitPt = Vector2();
    hitPt.X = 0.0f;
    hitPt.Y = 0.0f;
...
int Body::getClosestPointMass(Vector2 pos, float &dist)
{
    ...
    dist = Vector2::fsqrtf(closestSQD);
    return closest;
}
...
void Body::addGlobalForce(Vector2 pt, Vector2 force)
{
	...
	Vector3 RC=VectorTools::vec3FromVec2(R);

	float torqueF = Vector3::Cross(RC, VectorTools::vec3FromVec2(force)).Z;

    for (unsigned int i = 0; i < mPointMasses.size(); i++)
    {
        Vector2 toPt = mPointMasses[i]->Position - mDerivedPos;
		Vector2 torque = VectorTools::rotateVector(toPt, -(thePI / 2));
